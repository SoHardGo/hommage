...
// Vérification si l'utilisateur à fournit son adresse pour l'expédition
        ...
        if(!$info_user['number_road'] || !$info_user['address'] || !$info_user['city'] || !$info_user['postal_code']){
            $message = '<p class="message">Vos coordonnées ne sont pas complètes, veuillez modifier votre compte pour pouvoir valider l\'envoi à votre domicile</p>';
        } else {
            $message = 'Adresse de destination  :'.$info_user['number_road'].' '.$info_user['address'].' '.$info_user['postal_code'].' '.$info_user['city'];
        }
// Choix du défunt et vérification si l'utilisateur qui a crée la fiche accepte de recevoir des bouquets
        if (isset($_POST['select_def']) && !empty($_POST['select_def'])){
            ...
            $user_defunct = $globalClass->verifyUserAdmin($defunct_info['user_id'])->fetch();
            if($user_defunct['flower']){
                $message = 'Envoi de vos bouquets à : '.$user_defunct['lastname'].' '.$user_defunct['firstname'];
            } else {
                $message = '<p class="message">L\'administrateur de la fiche ne souhaite pas recevoir de bouquets.</p>';
            }
        }
